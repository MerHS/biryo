...
object HTMLRenderer {
  ...
  def deleteExternalTag(s: String): String = {
    s.replaceAll(
      "<embed(.|\\v)*</embed>|<iframe(.|\\v)*</iframe>|<param(.|\\v)*</param>",
      ""
...
class HTMLRenderer(private val katex: KatexRenderer) {
  ...
  def renderMapper: NamuMap = {
    ...
    case DocLink(href: ExternalHref, alias) =>
      HTMLString(externalLinkRenderer(href, alias))
    ...
    case Include("틀:루비", args) =>
      ...
      HTMLString(
        s"<ruby><rb>$value</rb><rp>(</rp><rt>$ruby</rt><rp>)</rp></ruby>"
      )
    case i @ Include(s, _) if !useInlineCSS && s.startsWith("틀:") =>
      // TODO: check it in premap (산사나무)
      val id = escapeHTMLStr(s.substring(2))
        .replaceAll(" ", "_") + '-' + includes.length
  ...
  def includeScriptRenderer(inc: Include, id: String): String = {
    ...
    val args =
      for ((k, v) <- inc.args) yield s"'${escapeJSStr(k)}': '${escapeJSStr(v)}'"
  ...
  protected def footnotesRenderer(fns: List[FootNote]): String = {
    if (fns.isEmpty)
      ...
      s"<div ${c(footnoteListClass)}>" +
        fns
          .map(f =>
            ReverseFootNote(f.value, f.noteStr)
  ...
  protected def headingItemRenderer(head: Headings): String = {
    ...
    val newHead = (head postMap { case NamuAST.FootNote(_, _) =>
      NamuAST.RawString("")
    ...
    val hno = no.mkString(".")
    val itemStr =
      s"<div ${c(ctItemClass)}><a href=${toQ(s"entry://#s-$hno")}>$hno.</a> ${value.mkString}</div>"
  ...
  protected def externalLinkRenderer(
      href: ExternalHref,
      alias: Option[NamuMark]
  ): String = {
    alias match {
      case Some(nm) =>
  ...
  case class ReverseFootNote(value: NamuMark, noteStr: Option[String])
      extends HasNamu {
    override def mkString = {
      val content = noteStr match {
        case Some(s) =>
          s"<a name=${toQ(s"fn-$s")}></a><a href=${toQ(s"entry://#rfn-$s")}>[$s]</a> ${value.mkString}"
        case None =>
          s"<a name=${toQ("fn-WTF")}></a><a href=${toQ("entry://#rfn-WTF")}>[*]</a> ${value.mkString}"
      }
