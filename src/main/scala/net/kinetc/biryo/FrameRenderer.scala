...
class FrameRenderer(private val katex: KatexRenderer) extends HTMLRenderer(katex) {
  ...
  case class FrameFootnote(value: NamuMark, noteStr: Option[String]) extends HasNamu {
    override def mkString = noteStr match {
      case Some(s) => s"<a name=${toQ(s"fr$s")}></a><a href=${toQ(s"entry://#f$s")}>[$s]</a>"
      case None => s"<a name=${toQ("frWTF")}></a><a href=${toQ("entry://#fWTF")}>[*]</a>"
    }

    def constructor(nm: NamuMark) = FootNote(nm, noteStr)
  }

  case class FrameReverseFootNote(value: NamuMark, noteStr: Option[String]) extends HasNamu {
    override def mkString = {
      val content = noteStr match {
        case Some(s) => s"<a name=${toQ(s"f$s")}></a><a href=${toQ(s"entry://#fr$s")}>[$s]</a> ${value.mkString}"
        case None => s"<a name=${toQ("fWTF")}></a><a href=${toQ("entry://#frWTF")}>[*]</a> ${value.mkString}"
      }
