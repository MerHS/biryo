...
object NamuAST {
  ...
  trait NamuMark {
    ...
    /** Depth-First Post-Order Node Mapping
      ...
      */
    def postMap(f: NamuMap): NamuMark =
  ...
  implicit def trav2Option[K <: NamuMark](x: Iterable[K]): Option[K] =
    x.headOption

  implicit def trav2Seq[K <: NamuMark](x: Iterable[K]): Seq[K] = x.toSeq

  /** A Class that has a derived type or sequence of NamuMark
    * @param valueSeq Iterable NamuMark Values
    * @tparam K Child of NamuMark
    * @tparam T can be Iterable implicitly (e.g. Option, Seq, ...)
    */
  abstract class HasNamuSeq[+K <: NamuMark, T](valueSeq: T)(implicit
      e: T => Iterable[K],
      revE: Iterable[K] => T
  ) extends NamuMark {
    ...
    override def postMap(f: NamuMap) = {
      val childMap = valueSeq.map(_.postMap(f)).asInstanceOf[Iterable[K]]
    ...
    override def map(f: NamuMap): HasNamuSeq[K, T] = {
      val mapped = valueSeq.map(_.preMap(f)).asInstanceOf[Iterable[K]]
  ...
  case class RawHref(value: String, override val escapeValue: String)
      extends NamuHref
  ...
  /** List of NamuMark Objects (`Seq[NamuMark]` Wrapper)
    ...
    */
  case class Paragraph(valueSeq: Seq[NamuMark])
      extends HasNamuSeq[NamuMark, Seq[NamuMark]](valueSeq) {
    def constructorSeq(nm: Seq[NamuMark]) = Paragraph(nm)
  }

  case class ParagraphBuilder(markList: Seq[NamuMark], sb: StringBuilder)
      extends NamuMark

  /// ===== Indent & Lists ===== ///

  case class ListObj(value: NamuMark, listType: ListType, indentSize: Int)
      extends HasNamu {
  ...
  case class BgColor(value: String, forTable: Boolean)
      extends TableCSS
      with ForTable {
  ...
  case class BorderColor(value: String, forTable: Boolean)
      extends TableCSS
      with ForTable {
    def style: String = s"border:2px solid $value"
  }

  case class Width(value: String, forTable: Boolean)
      extends TableCSS
      with ForTable {
    def style: String = s"width:${toPx(value)}"
  }

  case class Height(value: String, forTable: Boolean)
      extends TableCSS
      with ForTable {
    def style: String = s"height:${toPx(value)}"
  }

  case class Align(align: TableAlign, forTable: Boolean)
      extends TableCSS
      with ForTable {
    def style: String =
      if (forTable) {
  ...
  case class RowSpan(value: Int, align: TableAlign)
      extends TableCSS
      with TableAttr {
  ...
  trait HasTableStyle {
    ...
    def attrStr: String = {
      val attributes =
        styles.filter(_.isInstanceOf[TableAttr]).map(_.asInstanceOf[TableAttr])
    ...
    def styleStr: String = {
      val inlineStyles =
        styles.filter(_.isInstanceOf[TableCSS]).map(_.asInstanceOf[TableCSS])
  ...
  case class Table(valueSeq: Seq[TR], styles: Seq[TableStyle])
      extends HasNamuSeq[TR, Seq[TR]](valueSeq)
      with HasTableStyle {

    def constructorSeq(nm: Seq[TR]) = Table(nm, styles)
    override def mkString =
      s"<table $attrStr $styleStr ${c(tableClass)}><tbody>$trString</tbody></table>"
  ...
  case class TR(valueSeq: Seq[TD], styles: Seq[TableStyle])
      extends HasNamuSeq[TD, Seq[TD]](valueSeq)
      with HasTableStyle {
  ...
  case class BlockQuote(value: NamuMark) extends HasNamu {
    override def mkString =
      s"<blockquote><div ${c(indentClass)}>${value.mkString}</div></blockquote>"
  ...
  case class FootNote(value: NamuMark, noteStr: Option[String])
      extends HasNamu {
    override def mkString = noteStr match {
      case Some(s) =>
        s"<a name=${toQ(s"rfn-$s")}></a><a href=${toQ(s"entry://#fn-$s")}>[$s]</a>"
      case None =>
        s"<a name=${toQ("rfn-WTF")}></a><a href=${toQ("entry://#fn-WTF")}>[*]</a>"
    }
  ...
  case class LinkOnlyFN(noteStr: String) extends NamuMark {
    override def mkString =
      s"<a href=${toQ(s"entry://#fn-$noteStr")}>[$noteStr]</a>"
  }
  ...
  case class PageCount(namespace: String) extends NamuMark {
    override def mkString =
      s"<span ${c(s"pc-$namespaceClass")}>[$namespace 개수]</span>"
  ...
  case class Include(rawHref: String, args: Map[String, String])
      extends NamuMark {
    override def mkString = {
  ...
  case class YoutubeLink(id: String, args: Map[String, String])
      extends NamuMark {
    // Fallback to Link (For MDict)
    override def mkString =
      s"<a href=${toQ(s"entry://https://www.youtube.com/watch?v=$id")}>[유튜브 링크]</a>"
  }

  case class KakaoLink(id: String, args: Map[String, String]) extends NamuMark {
    // Fallback to Link (For MDict)
    override def mkString =
      s"<a href=${toQ(s"entry://https://tv.kakao.com/v/$id")}>[유튜브 링크]</a>"
  }

  case class NicoLink(id: String, args: Map[String, String]) extends NamuMark {
    // Fallback to Link (For MDict)
    override def mkString =
      s"<a href=${toQ(s"entry://https://www.nicovideo.jp/watch/$id")}>[유튜브 링크]</a>"
  }

  case class RubyMacro(word: String, ruby: String, color: Option[String])
      extends NamuMark {
    override def mkString = color match {
      case Some(c) =>
        s"<ruby><rb>$word</rb><rp>(</rp><rt><span style=${toQ(c)}>$ruby</span></rt><rp>)</rp></ruby>"
      case None =>
        s"<ruby><rb>$word</rb><rp>(</rp><rt>$ruby</rt><rp>)</rp></ruby>"
    }
  }
  ...
  // [[파일:$href|$htmlOption]]
  case class FileLink(href: String, htmlOption: Map[String, String])
      extends NamuMark {
    // Fallback to Link (for Mdict)
  ...
  case class Redirect(href: NamuHref) extends NamuMark with HasHref {
    ...
    override def mkString =
      s"<a href=${toQ(href.escapeValue)}>리다이렉트:${href.value}</a>"
  }
  ...
  case class InlineString(value: String, isMultiLine: Boolean)
      extends NamuMark {
    override def mkString =
      if (isMultiLine) s"<pre><code>$value</code></pre>"
      else s"<code>$value</code>"
  }
  // [br] -> BR
  ...
  private val dateMacroFormatter =
  ...
  private[biryo] def pbMerger(
      pb: ParagraphBuilder,
      lineObj: NamuMark
  ): ParagraphBuilder = {
    if (pb.sb.isEmpty) {
      pb.markList match {
        case (p: Paragraph) +: Seq() =>
          ParagraphBuilder(p.valueSeq :+ lineObj, pb.sb)
